...
def init_config():
    ...
    if _key_path and not os.path.exists(_key_path):
        raise error.MujocoDependencyError(
            "MUJOCO_PY_MJKEY_PATH path does not exist: {}".format(_key_path)
        )

    mjpro_path = os.environ.get("MUJOCO_PY_MJPRO_PATH")
    if mjpro_path and not os.path.exists(mjpro_path):
        raise error.MujocoDependencyError(
            "MUJOCO_PY_MJPRO_PATH path does not exist: {}".format(mjpro_path)
        )
    ...
    if not _key_path and not mjpro_path:
        ...
        raise error.MujocoDependencyError(
            "Found your MuJoCo binaries but not license key. Please put your key into ~/.mujoco/mjkey.txt or set MUJOCO_PY_MJKEY_PATH. Follow the instructions on https://github.com/openai/mujoco-py for setup."
        )
    elif not mjpro_path:
        raise error.MujocoDependencyError(
            "Found your MuJoCo license key but not binaries. Please put your binaries into ~/.mujoco/mjpro131 or set MUJOCO_PY_MJPRO_PATH. Follow the instructions on https://github.com/openai/mujoco-py for setup."
        )
...
def check_mujoco_version():
    ...
    if mjpro != "mjpro131":
        raise error.MujocoDependencyError(
            "We expected your MUJOCO_PY_MJPRO_PATH final directory to be 'mjpro131', but you provided: {} ({}). MuJoCo often changes in incompatible ways between versions, so you must use MuJoCo 1.31. If you're using MuJoCo 1.31 but changed the directory name, simply change the name back.".format(
                mjpro, mjpro_path
            )
        )
